(function() {
  ...
  function getLanguage() {
    ...
    // Spanish is specified or no language is set and browser is set to spanish
    if (options.language === 'mx' || (!options.language && navigator && navigator.language.match(/^mx/))) {
      language = 'es';
    }
    ...
    // China is specified or no language is set and browser is set to German
    if (options.language === 'cn' || (!options.language && navigator && navigator.language.match(/^cn/))) {
      language = 'cn';
    }
    ...
    // Finnish is specified or no language is set and browser is set to Dutch
    if (options.language === 'fi' || (!options.language && navigator && navigator.language.match(/^fi/))) {
      language = 'fi';
    }
    ...
    // Russian is specified or no language is set and browser is set to Dutch
    if (options.language === 'ru' || (!options.language && navigator && navigator.language.match(/^ru/))) {
      language = 'ru';
    }

    // Norwegian is specified or no language is set and browser is set to Dutch
    if (options.language === 'no' || (!options.language && navigator && navigator.language.match(/^no/))) {
      language = 'no';
    }
  ...
  function maximize() {
    ...
    document.getElementById(DOM_ID).style.bottom = '0';
  }
  ...
  function initializeInterface() {
    ...
    injectCSS('DIGITAL_STRIKE_CSS',
      '#' + DOM_ID + ' { position: fixed; right: 0; left: 0; bottom: 30px; width: 100%; height: ' + iFrameHeight + '; z-index: 20000; -webkit-overflow-scrolling: touch; overflow: hidden; } ' +
